...
- **마이하우스는 "AM"(저압냄비) 서브브랜드도 포함** — 2026-08-03에 이 별칭이 빠져서 경쟁사 편성을
  "0건·소강"으로 잘못 판단한 적 있음(형님이 직접 발견). `contains_any`는 단어경계 매칭
  (`(?<![A-Za-z0-9가-힣])별칭(?![A-Za-z0-9가-힣])`)으로 수정돼 있어 "AM" 같은 짧은 영문 별칭을 추가해도
  "팬암/아만/CAM" 같은 오탐은 나지 않음. **짧은 영문 별칭을 새로 추가할 때도 이 매칭 방식 덕분에 안전
  하지만, 그래도 추가 후엔 반드시 실제 매칭된 행을 한 번 찍어서 확인할 것.**
- **수동 갱신 필요**: `risk`(침투율로 자동 산출됨)와 달리 `trendLabel`, `flags`는 스크립트가 빈 값으로만
  채움 — 매주 지미가 수치 보고 직접 채워넣어야 함 (오버뷰 `weeklyCompareInsights`와 같은 패턴). `penetration`도
  최신월이 데이터 며칠치뿐이면(예: 8월 2일치) 왜곡되니 그럴 땐 직전 완결월 기준으로 수동 재계산해서 덮어쓸 것.
